        ...
        <ul className="grid w-full gap-6 md:grid-cols-2">
            {dataSource.map((platform) => (
                <li key={platform.id}>
                    ...
                    <label
                        htmlFor={platform.id}
                        className={`inline-flex items-center relative justify-between w-full p-5 text-gray-500 bg-white border border-gray-200 rounded-lg cursor-pointer
                             dark:border-gray-700 dark:peer-checked:text-blue-500 peer-checked:border-blue-600 dark:peer-checked:border-blue-600
                              peer-checked:text-blue-600 dark:text-gray-400 dark:bg-gray-800 ${platform.isActive && ('dark:hover:bg-gray-700 hover:bg-gray-100 dark:hover:text-gray-300 hover:text-gray-600')}`}
                    >
                        {!platform.isActive && (
                            <div className="backdrop-blur-xs w-full h-full absolute z-30 top-0 right-0"></div>
                        )}
                        <div className="block">
